...
(** Apply a list of non overlapping text edits. The order of application matters when
    multiple inserts are done in the same position. All the offsets are interpreted
    relative to the original document. *)
val apply_text_document_edits : t -> TextEdit.t list -> t

(** [absolute_position t pos] returns the absolute position of [pos] inside [text t]. If
    the position is outside the bounds of the document, the offset returned will be the
