...
(** Convert a thunk to a Memo computation, making sure the thunk runs in the context of
    the Memo computation rather than in the current context. [of_thunk f] is equivalent to
    [return () >> f] but is more explicit. *)
...
module O : sig
  (** [>>>] is a sequencing operator. [a >>> b] is the fiber that first executes [a] and
      then [b]. *)
  val ( >>> ) : unit t -> 'a t -> 'a t

  (** [>>=] is similar to [>>>] except that the result of the first fiber is used to
...
(* Jane Street: [ppx_let] support. Added because this vendored copy replaces the opam
   [fiber] package, which provided [Let_syntax]. *)
...
(** Execute a list of fibers in sequence. We use the short name to conform with the
...
(** The following functions combine forking 2 or more fibers followed by joining the
    results. The execution of the various fibers might be interleaved, however once the
    combining fiber has terminated, it is guaranteed that there are no fibers lingering
    around. *)

(** Start two fibers and wait for their result. Note that this function combines both
    successes and errors: if one of the computations fails, we let the other one run to
    completion, to give it a chance to raise its errors too. All other parallel execution
    combinators have the same error semantics. *)
...
(** Like [all_concurrently] but is specialized for [unit] fibers. The advantage being that
    it doesn't allocate a return list. *)
...
(** [finalize f ~finally] runs [finally] after [f ()] has terminated, whether it fails or
    succeeds. *)
...
module Ivar : sig
  ...
  (** Fill the ivar with the following value. This can only be called once for a given
      ivar. *)
...
(** Mailbox variables *)
module Mvar : sig
  (** A mailbox variable can be thought of as a box that is either empty or full.
      [create ()] creates a new empty box, and [create_full x] creates a new full box
      containing [x].

      [read] removes the value from a full mailbox variable and returns it, but blocks if
      the mvar is currently empty. Symmetrically, [write] puts a value into the mvar but
...
(** State variables *)
module Svar : sig
  (** A "state variable" [Svar.t] differs from [Ivar.t] in that it can be updated multiple
      times. It is particularly useful if you'd like to observe the state of a process by
      subscribing to "interesting" (from your perspective) events.

      For example, a build system can have a complex state machine, switching between
      states like "waiting for source file changes", "building", etc., by calling
      [Svar.write] on the corresponding state variable. If you don't care about most of
      the states but would like to observe all errors, you can use
      [Svar.wait ~until:is_error] to be woken up when an error happens. Other observers
      may have different preferences, expressed with different [until] predicates. You can
      also use the non-blocking [Svar.read] if you'd like to use polling instead. *)
  ...
  (** [wait t ~until] waits until [t]'s value satisfies the predicate [until]. If the
      current value satisfies it, [wait] returns immediately, otherwise, the caller is
      blocked. *)
...
module Stream : sig
  (** Destructive streams that can be composed to pipelines.
      ...
      Streams can be finite or infinite. Streams have no storage and can only have one
      writer and/or one reader at any given time. If you'd like to access a stream
      concurrently, you need to protect it via a mutex.
  ...
  module In : sig
    (** Stream inputs.
        ...
        A [In.t] value represents the side of a stream where values can be read from. A
        stream input can only be consumed by one fiber at a time. Trying to access a
        stream input concurrently will result in an exception.

        When passing a stream input through one of the transformation functions such as
        {!filter_map} or iteration function, the original stream input can no longer be
        used for another purpose. *)
    type 'a t

    (** Create a stream that is fed from a generator function. Every time a value is
        requested, this function is called to produce a new value. If the function raises,
    ...
    (** Consumes and returns the next element from the stream. Once the stream is
        exhausted, [read] always returns [None]. *)
  ...
  module Out : sig
    (** Stream outputs.
        ...
        A [Out.t] value represents the side of a stream where values can be pushed to.
        Only one value can be pushed at a time. Trying to push two values concurrently
        will result in an error. *)
    type 'a t

    (** Create a stream output. The callback is the consumer for values pushed to the
        stream. *)
  ...
  (** [connect i o] reads from [i] and writes to [o]. Closes [o] when [i] is exhausted.
      Returned fiber terminates when [i] is exhausted *)
  val connect : 'a In.t -> 'a Out.t -> unit fiber

  (** [supply i o] like [connect i o] but does not close [o] once [i] is exhausted,
      allowing more values to be pused to [o]. Returned fiber terminates when [i] is
      exhausted *)
...
module Pool : sig
  ...
  (** [task pool ~f] submit [f] to be done in [pool]. Errors raised [pool] will not be
      raised in the current fiber, but inside the [Pool.run] fiber.

      If [running pool] returns [false], this function will raise a [Code_error]. *)
  val task : t -> f:(unit -> unit fiber) -> unit fiber

  (** [close pool] stops the pool from receiving new tasks. After this function is called,
      [task pool ~f] will fail to submit new tasks.

      Note that stopping the pool does not prevent already queued tasks from running.

      [close pool] subsequent calls to [close] ignored. In other words, this function is
      idempotent *)
  val close : t -> unit fiber

  (** [run pool] Runs all tasks submitted to [pool] in parallel. Errors raised by such
      tasks must be caught here. *)
...
(** [run t ~iter] runs a fiber until it terminates. [iter] is used to implement the
    scheduler, it should block waiting for an event and return at least one ivar to fill. *)
...
module Scheduler : sig
  ...
  (** [start t] starts executing a fiber. This advance the execution of the fiber as
      possible, until no more progress can be made. *)
  val start : 'a fiber -> 'a step

  (** Advance a stalled fiber as much as possible by filling a list of ivars. Once must
      call [advance] on a given [stalled] value only once. *)
  val advance : 'a stalled -> fill list -> 'a step
end

(** {1 Fiber cancellation} *)
module Cancel : sig
  (** This module provides a way to cancel long running computations. Cancellation is
      fully explicit and fibers must explicitly check for it at strategic points. *)
  ...
  (** Version of [fire] that is suitable to call from the [iter] callback of [Fiber.run]. *)
  ...
  (** [with_handler t ~on_cancel f] runs [f ()] with a cancellation handler. If [t] is
      fired during the execution of [f], then [on_cancel] is called.

      The aim of [on_cancel] is to somehow cut short the execution of [f]. A typical
      example is a function running an external command. [on_cancel] might send a [KILL]
      signal to the command to abort its execution.

      If [f ()] finished before [t] is fired, then [on_cancel] will never be invoked. *)
  val with_handler
    :  t
    -> (unit -> 'a fiber)
...
module Expert : sig
  ...
  (** [resume k a] resume a suspended fiber with [a]. It is forbidden to call [resume]
      more than once on a a fiber. *)
