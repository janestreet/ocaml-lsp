...
type runner =
  ...
  | Awaiting_resume of unit k
    (* Ran out of work. Waiting to be resumed once work is added or pool is closed. *)
...
(* A pool consumes tasks from a queue in the context where [run] was executed.
   ...
   It's implemented by a simple queue of thunks and a continuation to resume [run]
...
let run t k =
  ...
  | Awaiting_run ->
    t.runner <- Running;
    (* The number of currently running fibers in the pool. Only when this number reaches
       zero we may call the final continuation [k]. *)
    let n = ref 1 in
    let done_fiber () =
      decr n;
