...
let get_pos ~(params : Jsonrpc.Structured.t option) =
  match params with
  | Some (`Assoc [ ("index", _); ("verbosity", _); ("textDocument", _); ("at", at_json) ])
    ->
    (match Request_params.at_of_yojson at_json with
     | `Position p -> Some p
     | `Range r -> Some r.end_)
  | _ -> None
;;
...
let with_pipeline ~log_info state uri verbosity with_pipeline =
...
let on_request ~log_info ~params state =
  Fiber.of_thunk (fun () ->
    ...
    with_pipeline ~log_info state uri verbosity
