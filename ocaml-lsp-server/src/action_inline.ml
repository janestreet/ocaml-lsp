...
(** [find_parsetree_loc pl loc] finds an expression node in the parsetree with location
    [loc] *)
...
(** Mapping from [Location.t] to [Path.t]. Computed from the typedtree. Useful for
    determining whether two parsetree identifiers refer to the same path. *)
module Paths : sig
  type t
...
(** Rough check for expressions that can be duplicated without duplicating any side
    effects (or introducing a sigificant performance difference). *)
