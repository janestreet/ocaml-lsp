...
let build_progress t (progress : Drpc.Progress.t) =
  Fiber.of_thunk (fun () ->
    match t with
    | Disabled -> Code_error.raise_s [%message "progress reporting is not supported"]
    | Enabled ({ token; report_progress; _ } as t) ->
      (match progress with
       ...
       | In_progress progress ->
         ...
         (* The percentage is useless as it isn't monotinically increasing as the spec
            requires, but it's the best we can do. *)
         let percentage =
